...
// Lazy initialization - only define the model when database is connected
let UserEmbedding = null;

function getUserEmbeddingModel() {
  if (!UserEmbedding) {
    const sequelize = dbConfig.getSequelize();
    if (!sequelize) {
      throw new Error('Database not connected. Please ensure database connection is established before using models.');
    }
    
    UserEmbedding = sequelize.define('UserEmbedding', {
      id: {
        type: DataTypes.UUID,
...
module.exports = getUserEmbeddingModel;
